...
def get_features_by_name(load_name, year):
    ...
    cur = conn.cursor()
    _conn = sqlite3.connect("中压负荷相邻数据表.db")
    _cur = _conn.cursor()

    features = np.zeros((4, 12, 24))
    auto_encoder = get_autoencoder2(r"D:\OneDrive\桌面\毕设\代码\计及负荷异常增长的空间负荷预测与配电网规划\2.基于集成学习的空间负荷预测\AutoEncoder_20230125_173655.path")
...
if __name__ == '__main__':
    cnn = get_stacking_cnn(r"D:\OneDrive\桌面\毕设\代码\计及负荷异常增长的空间负荷预测与配电网规划\2.基于集成学习的空间负荷预测\stacking_CNN_20230212_191903.path")
